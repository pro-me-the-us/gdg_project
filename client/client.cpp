    ...
    void ChatInputThread(ENetPeer* serverpeer){
        std::string MyMsg;
        
        while(true){
            std::getline(std::cin,MyMsg);
            if(!MyMsg.empty()){
                ENetPacket* packet = enet_packet_create(MyMsg.c_str(),MyMsg.length()+1,ENET_PACKET_FLAG_RELIABLE);
    ...
    int main(){
        ...
        std::thread inputThread(ChatInputThread,peer);
        inputThread.detach();        
            ...
            while(enet_host_service(Client,&event,0)>0){
                switch(event.type){
                    case ENET_EVENT_TYPE_RECEIVE:{
                        std::string OtherMsg = (char*)event.packet->data;
                        std::cout << "Other's Msg : "<<OtherMsg<<"\n";
